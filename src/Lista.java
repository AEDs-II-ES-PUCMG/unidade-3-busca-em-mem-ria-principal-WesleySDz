...
public class Lista<E> implements IMedicao {
	...
	/**
	 * Insere um novo elemento na posição indicada. A posição máxima é o tamanho da
	 * lista e a mínima é 0.
	 * 
	...
	/**
	 * Remove um elemento da posição indicada. Se a lista estiveer vazia ou a
	 * posição for inválida (<0 ou >=tamanho),
	...
	/**
	 * Retorna, sem retirar, um elemento na posição indicada pelo parâmetro. A
	 * primeira posição da lista é
	 * considerada posição 0 e, assim, a última é (tamanho-1). Lança exceções para
	 * lista vazia ou posições inválidas.
	 * 
	...
	/**
	 * Localiza um elemento na lista de acordo com um Predicado. Retorna o primeiro
	 * elemento na lista
	 * que atende àquele predicado ou nulo caso não exista. Causa exceções em listas
	 * vazias ou no caso
	 * de não haver elementos satisfazendo o predicado.
	 * 
	 * @param condicional Predicado com a condição para encontrar um elemento.
	 * @return O primeiro elemento encontrado que atenda à condição ou nulo, caso
	 *         não haja.
	 * @throws IllegalStateException  se a lista estiver vazia
	 * @throws NoSuchElementException se não houver nenhum elemento satisfazendo a
	 *                                condição.
	 */
	 ...
	 * @throws NoSuchElementException se não existe elemento na lista igual ao
	 *                                procurado
	 */
	...
	/**
	 * Conta quantos elementos na lista atendem à condição estabelecida pelo
	 * predicado.
	 * 
	 * @param condicional Predicado com a condição para verificação de elementos na
	 *                    lista
	 * @return inteiro com a quantidade de elementos que atendem ao predicado
	...
	/**
	 * Calcula e retorna o valor médio de um determinado atributo dos elementos da
	 * fila,
	 * utilizando uma função de extração fornecida.
	 *
	 * @param extrator   uma função que extrai um valor numérico (Double) de cada
	 *                   elemento da fila.
	 * @param quantidade a quantidade de elementos a serem considerados para o
	 *                   cálculo da média;
	 *                   se for maior que o tamanho da fila, será ajustada para o
	 *                   tamanho da fila.
